...
class Kettle():
    ...
    def __init__(self, args, batch_size, augmentations, setup=dict(device=torch.device('cpu'), dtype=torch.float)):
        ...
        print(f"self.trainset[1]: {self.trainset[1]}")
        """
        ######################################################################
        ...
        print(f"poisonloader got...: {self.poisonloader}")
        print(f"poisonset: {self.poisonset}")
    ...
    def prepare_data(self, normalize=True):
        ...
        if self.augmentations is not None or self.args.paugment:
            ...
            elif 'Imagenette' in self.args.dataset:
                params = dict(source_size=160, target_size=160, shift=160 // 4, fliplr=True)
...
########################################################################################################################
    ...
    def _choose_poisons_deterministic(self, target_id):
        ...
        if len(class_ids) < poison_num:
            ...
            poison_num = len(class_ids)
        self.poison_ids = class_ids[:poison_num] # we simply take the first poison_num images from the target class
        ...
        for index in range(len(self.validset)):
            ...
            if idx not in self.target_ids:
                valid_indices.append(idx)
        validset = Subset(self.validset, indices=valid_indices)
        poisonset = Subset(self.trainset, indices=self.poison_ids)  # {1357: 0, 27541: 1, 764: 2, ... xxx: 499}
    ...
    def _choose_poisons_randomly(self):
        ...
        if self.poison_setup['poison_class'] is not None:
            ...
            for index in range(len(self.trainset)):  # we actually iterate this way not to iterate over the images
                ...
                print(f"idx: {idx}")
                print(f"poison_setup['poison_class']: {poison_setup['poison_class']}")
                """
                if target == self.poison_setup['poison_class']:
                    class_ids.append(idx)
    ...
    def export_poison(self, poison_delta, path=None, data_path=None, mode='automl'):
        ...
        if data_path is None: # save full celan images (prepare for unlearning)
        ...
        def _save_image(input, label, idx, location, train=True):
            ...
            if (lookup is not None) and train:
                ...
                # save poison indices
                # self.poison_idx_saved[self.poison_count] = idx
                # self.poison_count += 1

            _torch_to_PIL(input).save(filename)

        def _save_clean_image(input, label, idx, location, train=True):
            """Save input image to given location, WITHOUT poison_delta!"""
            filename = os.path.join(location, str(idx) + '.png')
            _torch_to_PIL(input).save(filename)
        ...
        if mode == 'packed':
            ...
            # check device
            for input, label, idx in self.trainset:
                print(f"self.trainset.device: {input.device}") # cpu
                break
            """
            # print(f"poison_delta.device: {poison_delta.device}") # cuda:0
            # send to the same device
            poison_delta = poison_delta.cpu()
            # save poison_ids
            data = dict()
            data['poison_setup'] = self.poison_setup
            data['poison_delta'] = poison_delta
            data['poison_ids'] = self.poison_ids
            data['target_images'] = [data for data in self.targetset]
            save_path = os.path.join(path, 'poison_info.pkl')
            os.makedirs(path, exist_ok=True)
            with open(save_path, 'wb') as file:
                pickle.dump(data, file) 
            
            # save full training images
            names = self.trainset.classes
            for name in names:
                os.makedirs(os.path.join(path, 'train', name), exist_ok=True)
            ...
            names = self.trainset.classes
            for name in names:
                os.makedirs(os.path.join(cleans_path, 'train', name), exist_ok=True)
                os.makedirs(os.path.join(cleans_path, 'test', name), exist_ok=True)
                os.makedirs(os.path.join(cleans_path, 'targets', name), exist_ok=True)
            for input, label, idx in self.trainset:
                _save_clean_image(input, label, idx, location=os.path.join(cleans_path, 'train', names[label]), train=True)
            print('Original clean training images exported ...')
            
            # np.save(self.save_poison_idx_path, self.poison_idx_saved)
            # print('poison idx saved...')

            for input, label, idx in self.validset:
                _save_clean_image(input, label, idx, location=os.path.join(cleans_path, 'test', names[label]), train=False)
            print('Unaffected test images exported ...')

            # Save secret targets
            for enum, (target, _, idx) in enumerate(self.targetset):
                intended_class = self.poison_setup['intended_class'][enum]
                _save_clean_image(target, intended_class, idx, location=os.path.join(cleans_path, 'targets', names[intended_class]), train=False)
            print('Target images exported with intended class labels ...')
