...
def get_model(model_name, dataset_name, pretrained=False):
    ...
    if 'CIFAR' in dataset_name or 'MNIST' in dataset_name:
        ...
        if pretrained:
            raise ValueError('Loading pretrained models is only supported for Imagenette.')
        in_channels = 3
        num_classes = 10 
        if 'ResNet' in model_name:
            model = resnet_picker(model_name, dataset_name)
...
def resnet_picker(arch, dataset):
    ...
    num_classes = 10
    if dataset in ['CIFAR10', 'CIFAR10_BadNet_Patch', 'CIFAR10_Blend_Random', 'CIFAR10_ST_Debug', 'CIFAR10_Clean', 'CIFAR10_SmoothTrigger', 'CIFAR10_Filtered', 'CIFAR10_Poisoned', 'CIFAR10_500_Removed', 'CIFAR10_Portion_Removed', 'CIFAR10_Randomly_Removed', 'MNIST']:
        num_classes = 10
        initial_conv = [3, 1, 1]
    elif dataset in ['CIFAR100', 'CIFAR100_Filtered', 'CIFAR100_ST_Debug', 'CIFAR100_BadNet_Patch']:
        num_classes = 100
        initial_conv = [3, 1, 1]
    elif dataset == 'TinyImageNet':
        num_classes = 200
        initial_conv = [7, 2, 3]
    elif dataset in ['Imagenette', 'Imagenette_filtered', 'Imagenette_WB_filtered']:
        num_classes = 10
        initial_conv = [7, 2, 3]
    else:
    ...
    if arch == 'ResNet20':
        ...
        # ResNet(torchvision.models.resnet.BasicBlock, [2, 2, 2, 2], num_classes=10, base_width=64, initial_conv=[3, 1, 1])
...
class ResNet(torchvision.models.ResNet):
    ...
    def __init__(self, block, layers, num_classes=10, zero_init_residual=False,
        ...
        self.base_width = 64  # Do this to circumvent BasicBlock errors. The value is not actually used.   # for cifar, initial_conv = [3, 1, 1]
