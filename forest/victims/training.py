...
def run_step(kettle, poison_delta, loss_fn, epoch, stats, model, defs, criterion, optimizer, scheduler, ablation=True):
    ...
    for batch, (inputs, labels, ids) in enumerate(loader):
        ...
        if poison_delta is not None:
            ...
            if batch_positions: 
                inputs[batch_positions] += poison_delta[poison_slices].to(**kettle.setup) # add adv pattern onto the image
        ########################################################################
        ...
        ##### xxxx ####
        # Does adversarial training help against poisoning? (ignore this fn)
    ...
    # print(f"epoch_loss: {epoch_loss / (batch + 1)}")
    # print(f"epoch_acc: {epoch_loss / (correct_preds / total_preds)}")
