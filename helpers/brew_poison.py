...
import sys
sys.append('/data/xxx/github/poisoning-gradient-matching/opensource_ready/helpers')
import forest
...
if __name__ == "__main__":
    ...
    if args.pretrained:
        ...
        # clean data will be automatically downloaded at '--data_path' (default='~/data')
        stats_clean = model.train(data, max_epoch=args.max_epoch)
    train_time = time.time()

    # save the initial clean model
    model_path = args.model_path
    os.makedirs(model_path, exist_ok=True)
    # model.save_model(model_path + '/clean.pth')
    # print("(before poisoning) clean model saved in" + model_path + "/clean.pth")
    ...
    if not args.pretrained and args.retrain_from_init:
        ...
        print("=== if not args.pretrained and args.retrain_from_init: === (Not sure what this does)")
    else:
    ...
    # victim_model_path = args.model_path
    # os.makedirs(victim_model_path, exist_ok=True)
    model.save_model(model_path + '/victim.pth')
    print("victim model saved in" + model_path + "/victim.pth")
    
    # Export
    if args.save is not None:
        data.export_poison(poison_delta, path=args.poison_path, data_path=args.data_path, mode=args.save)
