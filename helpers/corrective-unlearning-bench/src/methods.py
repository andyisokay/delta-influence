...
class Naive():
    ...
    def train_one_epoch(self, loader):
        ...
        # for (images, target) in tqdm.tqdm(loader):
            ...
            infgt = infgt.cuda()  # for BadT
            with autocast():
                self.optimizer.zero_grad()
                loss = self.forward_pass(images, target, infgt) # for Scrub & BadT
                # loss = self.forward_pass(images, target)
    ...
    def save_results(self, test_loader):
        self.get_save_prefix()
        print(f"self.unlearn_file_prefix: {self.unlearn_file_prefix}")
        if not exists(self.unlearn_file_prefix):
            makedirs(self.unlearn_file_prefix)
        
        # save the unlearned model
        torch.save(self.best_model.state_dict(), self.unlearn_file_prefix+'/unlearned_model.pth')
        # np.save(self.unlearn_file_prefix+'/train_top1.npy', self.save_files['train_top1'])
        # np.save(self.unlearn_file_prefix+'/val_top1.npy', self.save_files['val_top1'])
        np.save(self.unlearn_file_prefix+'/unlearn_time.npy', self.save_files['train_time_taken'])
        self.model = self.best_model.cuda()
...
class Scrub(ApplyK):
    ...
    def get_save_prefix(self):
        self.unlearn_file_prefix = self.opt.pretrain_file_prefix+'/'+str(self.opt.deletion_size)+'_'+self.opt.unlearn_method+'_'+self.opt.exp_name+'_'+str(self.opt.train_iters)
        # self.unlearn_file_prefix += '_'+str(self.opt.train_iters)+'_'+str(self.opt.k)
...
class BadT(ApplyK):
    ...
    def forward_pass(self, images, target, infgt): # infgt tells if the current examples is in detected_idx
        if self.prenet is not None:
            with torch.no_grad():
                feats = self.prenet(images)
            output = self.model(feats)
            ...
            output = self.model(images)
        # print(f"images's device: {images.device}")
        # print(f"target's device: {target.device}")
        ...
        # print(f"u_teacher_out's device: {u_teacher_out.device}")
        # print(f"f_teacher_out's device: {f_teacher_out.device}")
        # print(f"labels's device: {labels.device}") <--- should be also on cuda
        overall_teacher_out = labels * u_teacher_out + (1-labels)*f_teacher_out # <--- key steps from here for BadT
        student_out = F.log_softmax(output /self.kltemp, dim=1)
        loss = F.kl_div(student_out, overall_teacher_out)
        contains_detected = (labels == 1).any()
        # print((labels == 1).sum().item())
        """
        if self.count == 0 and (labels == 1).sum().item()>=5:
            print("the current sample is in detected set...")
            print(f"overall_teacher_out's shape: {overall_teacher_out.shape}")
            print(f"overall_teacher_out: {overall_teacher_out}")
            print(f"student_out's shape: {student_out.shape}")
            print(f"student_out: {student_out}")
            print(f"loss: {loss}")
            self.count = -1
            # debug
            student_teacher_logits_save_path = '/data/xxx/github/poisoning-gradient-matching/open_source/smooth_trigger/cifar10/exp03_MM_N50/debug'
            np.save(student_teacher_logits_save_path+'overall_teacher_out.npy', overall_teacher_out.detach().cpu().numpy())
            np.save(student_teacher_logits_save_path+'student_out.npy', student_out.detach().cpu().numpy())
            np.save(student_teacher_logits_save_path+'ifdetected.npy', labels.detach().cpu().numpy())
        """
        # print(f"output's device: {output.device}")
        # print(f"target's device: {target.device}")
        self.top1(output, target)
        return loss
...
class SSD(ApplyK):
    ...
    def get_save_prefix(self):
        ...
        # self.unlearn_file_prefix += '_'+str(self.opt.train_iters)
        self.unlearn_file_prefix += '_'+str(self.opt.train_iters)+'_' + str(self.opt.SSDselectwt)+'_'+str(self.opt.SSDdampening)
        return 
...
class InfluenceWitchesBrew(Naive):
    def __init__(self, opt, model, prenet=None):
        super().__init__(opt, model, prenet)
        self.task = ClassificationTask()
        self.model = prepare_model(model=self.model, task=self.task)
